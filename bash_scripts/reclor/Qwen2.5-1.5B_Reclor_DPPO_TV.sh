...
# DAPO DPPO-TV Training Configuration
# DPPO-Binary-TV: Dual-clip PPO with Total Variation divergence
# Reference: https://arxiv.org/pdf/2602.04879

python3 -m verl.trainer.main_ppo \
    # DPPO-TV uses GRPO advantage but different policy loss
    algorithm.adv_estimator=grpo \
    # DPPO-specific: Filter Groups (Dynamic Sampling)
    algorithm.filter_groups.enable=True \
    algorithm.filter_groups.metric=acc \
    algorithm.filter_groups.max_num_gen_batches=10 \
    # DPPO-TV-specific: Total Variation divergence clipping
    actor_rollout_ref.actor.clip_ratio_low=0.2 \
    actor_rollout_ref.actor.clip_ratio_high=0.2 \
    actor_rollout_ref.actor.clip_ratio_c=20.0 \
    # DPPO-specific: token-mean loss aggregation
    ...
    trainer.experiment_name='Qwen2.5-1.5B_DPPO_TV_Reclor' \
