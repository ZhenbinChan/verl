...
# DAPO DPPO-KL Training Configuration
# DPPO-Binary-KL: Dual-clip PPO with KL divergence
# Reference: https://arxiv.org/pdf/2602.04879

python3 -m verl.trainer.main_ppo \
    # DPPO-KL uses GRPO advantage but different policy loss
    algorithm.adv_estimator=grpo \
    # DPPO-specific: Filter Groups (Dynamic Sampling)
    algorithm.filter_groups.enable=True \
    algorithm.filter_groups.metric=acc \
    algorithm.filter_groups.max_num_gen_batches=10 \
    # DPPO-KL-specific: KL divergence clipping
    ...
    trainer.experiment_name='Qwen2.5-1.5B_DPPO_KL_Reclor' \
