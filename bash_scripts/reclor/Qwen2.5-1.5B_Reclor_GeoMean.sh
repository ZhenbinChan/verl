...
# Geo-Mean (GMPO) Training Configuration
# Geometric Mean Policy Optimization
# Reference: https://arxiv.org/abs/2507.20673

python3 -m verl.trainer.main_ppo \
    # Geo-Mean uses GRPO advantage but geometric mean policy loss
    algorithm.adv_estimator=grpo \
    # Geo-Mean-specific: Filter Groups (Dynamic Sampling)
    algorithm.filter_groups.enable=True \
    algorithm.filter_groups.metric=acc \
    algorithm.filter_groups.max_num_gen_batches=10 \
    # Geo-Mean-specific: Asymmetric clipping
    actor_rollout_ref.actor.clip_ratio_low=0.2 \
    actor_rollout_ref.actor.clip_ratio_high=0.28 \
    # Geo-Mean uses standard token-mean (loss aggregation handled internally)
    ...
    trainer.experiment_name='Qwen2.5-1.5B_GeoMean_Reclor' \
