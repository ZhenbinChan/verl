...
INPUT_PARQUET=${INPUT_PARQUET:-${PROJECT_ROOT}/data/reclor/train.parquet}
OUTPUT_DIR=${OUTPUT_DIR:-${PROJECT_ROOT}/data/reclor_global_fol_prm}
OUTPUT_PARQUET_NAME=${OUTPUT_PARQUET_NAME:-train.parquet}
METADATA_FILENAME=${METADATA_FILENAME:-fol_metadata.json}
FAILURES_FILENAME=${FAILURES_FILENAME:-metadata_failures.json}
...
if [[ -n "${SAMPLE_ID_PREFIX}" ]]; then
  args+=(--sample_id_prefix "${SAMPLE_ID_PREFIX}")
fi
