"""IGRewardManager — reward manager for the information_gain sampling strategy.
...
1. **PRM reward_tensor** (main training signal, token-level):
   - If the batch already contains ``reward_fn_scores`` (placed there by
     :class:`~verl.trainer.ppo.sampling.information_gain.InformationGainStrategy`),
     those scores are used directly as the token-level reward.
   - Fallback (validation / non-IG batches): each ``<step>`` block is scored
     via the format/FOL PRM function.
...
class IGRewardManager:
    """Reward manager for the ``information_gain`` sampling strategy.

    Same structure as StepTreeRewardManager. Kept as a separate class for
    future customizations (e.g., recording per-step KL divergence values
    in ``reward_extra_info``).
    """
