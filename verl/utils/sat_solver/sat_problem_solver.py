...
class Z3_Program:
        ...
        for s in pure_declaration_statements:
            ...
            if "EnumSort" in s and (("Monday" in s or "monday" in s or "Tuesday" in s or "tuesday" in s)\
                ...
                for i, x in enumerate(sort_members):
                    tmp += (x + " == " + str(i+1) + ", ")
                variable_constrant_statements.append(tmp[:-2] + ")")
    ...
    def to_standard_code(self):
        ...
        for constraint in self.variable_constrants:
        ...
        return CodeTranslator.assemble_standard_code(declaration_lines, pre_condidtion_lines, option_blocks, self.dataset_name)
...
is_valid(Exists([m:meals], eats(Vladimir, m) == poached_eggs)) ::: (E)'''
    ...
    z3_program = Z3_Program(logic_program, 'AR-LSAT')
