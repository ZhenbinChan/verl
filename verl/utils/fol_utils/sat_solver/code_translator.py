...
class CodeTranslator:
    ...
    @staticmethod
    def assemble_standard_code(declaration_lines, pre_condidtion_lines, option_blocks, dataset_name):
        lines = []

        header_lines = [
            "from z3 import *", "import enum"
        ]
    ...
    @staticmethod
    def assemble_verifier_code(declaration_lines, variable_constraint_lines, pure_constraint_lines, declared_functions):
        lines = []
        ...
        lines += ["pre_conditions = []"]
        for line in variable_constraint_lines:
            if line.line_type == CodeTranslator.LineType.DECL:
                lines += [line.line]
            else:
                lines += ["pre_conditions.append({})".format(line.line)]
        ...
        t = 0
        for line in pure_constraint_lines:
            if line.line_type == CodeTranslator.LineType.DECL:
                lines += [line.line]
            else:
                if t == 0:
                    lines += ["condition_p = {}".format(line.line)]
                    t += 1
                elif t == 1:
                    lines += ["condition_q = {}".format(line.line)]
                else:
                    raise ValueError
        lines += [""]

        lines += [
            "def check_valid(pre_cond, cond1, cond2):",
            TAB_STR + "solver = Solver()",
            TAB_STR + "solver.add(pre_cond)",
            TAB_STR + "solver.add(cond1)",
            TAB_STR + "solver.add(Not(cond2))",
            TAB_STR + "print(solver.check() == unsat)",
            TAB_STR + "if solver.check() == sat:",
            TAB_STR + TAB_STR + "m = solver.model()"
        ]

        for name, args in declared_functions:
            for i in range(len(args)-1):
                lines += [TAB_STR*(i+2) + "for {} in {}:".format(args[i][0] + str(i), args[i])]
            string_format = ', '.join(['%s']*(len(args)-1))
            variable_format = ', '.join([args[i][0] + str(i) for i in range(len(args)-1)])
            lines += [TAB_STR*(len(args)+1) + f'print("%s: {string_format} -> %s"%({name}, {variable_format}, m.evaluate({name}({variable_format}))))']
        ...
        # call the function check_valid()
        lines += [
            "print('p->q')",
            "check_valid(pre_conditions, condition_p, condition_q)",
            "print('------')",
            "print('q->p')",
            "check_valid(pre_conditions, condition_q, condition_p)"
        ]
        return "\n".join(lines)