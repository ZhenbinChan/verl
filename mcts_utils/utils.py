...
def get_response(usr_input, system_prompt=None, assistant_output=None):
    client = OpenAI()
    messages = [
    ...
    for node in ast.walk(tree):
        ...
        for target in node.targets:
            ...
            if name.startswith(('premises_', 'conclusion_')):
                ptype, str_idx = name.split('_')[0], name.split('_')[-1]
                idx = int(str_idx) if str_idx.isdigit() else idx + 1
...
def construct_implication_prefix(parsed_implication):
    if not parsed_implication:
        return None
    code = "```python\n"
    for step in parsed_implication:
        idx = step['step_index']
        code += f"# Step {idx}\n"
        code += f"premises_{idx} = [\n"
        for premise in step['premises'][:-1]:
            code += f"    {premise},\n"
        code += f"    {step['premises'][-1]}\n]\n"
        code += f"conclusion_{idx} = {step['conclusion'][0]}\n\n"
    code += "```"
    return code
...
def verify_steps_nli(parsed_steps):
    ...
    original_levels = {}
    loggers_to_silence = ['transformers.pipelines.text_classification', 'transformers']
