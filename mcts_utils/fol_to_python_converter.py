...
class FOLToPythonConverter:
    ...
    def parse_constraints(self, constraints_text):
        ...
        for line in lines:
            ...
            if flag:
                constraints[-1] += ' ' + constraint  # 如果前面有空格，说明是上一行的续行，合并到上一行
            else:
...
if __name__ == "__main__":
    ...
    declarations = """
groups = EnumSort([Black_Americans, White_Americans, Westernized_Black_Africans, White_Africans])
condition = Function([groups] -> [bool])
hypertension_risk = Function([groups] -> [int])
salt_content = Function([bool] -> [int])
genetic_adaptation = Function([bool] -> [bool])
"""
    
    constraints = """
ForAll(g, Implies([g:group], And(And(Implies((g==Black_Americans), (hypertension_risk(g)==(2*hypertension_risk(White_Americans)))), Implies((g==Westernized_Black_Africans), (hypertension_risk(g)==(2*hypertension_risk(White_Africans))))), Implies((g==Black_Africans), (hypertension_risk(g)==(2*hypertension_risk(White_Africans)))))))    
"""
